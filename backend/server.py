...
async def _active_rental(org, did):
    return await db.driver_rentals.find_one({"organization_id": org, "driver_id": did, "status": {"$ne": "ended"}})


async def _rental_account(rental):
    """Recompute a driver's rental account server-side and persist for MyVolt admin. Single source of truth."""
    ...
    past = [start + timedelta(days=i) for i in range(max(ndays, 0))]  # excludes today
    paidrecs = await db.rental_payments.find({
        "organization_id": org, "driver_id": did,
        "kind": {"$in": ["daily", "outstanding"]}, "payment_status": "paid"}).to_list(3000)
    paid_dates = set()
    for p in paidrecs:
        for d in p.get("covers_dates", []):
            paid_dates.add(d)
    unpaid = [d.isoformat() for d in past if d.isoformat() not in paid_dates]
    overdue = len(unpaid)
    outstanding = overdue * rate
    status = "blocked" if overdue >= 2 else ("overdue" if overdue == 1 else "active")
    today_paid = today.isoformat() in paid_dates
    ...
    await db.rental_accounts.update_one(
        {"organization_id": org, "driver_id": did},
        {"$set": acct, "$setOnInsert": {"created_at": now_iso()}}, upsert=True)
    return acct
...
async def _driver_payload(user):
    ...
    out = {
        "driver": {"id": did, "name": user.get("name"), "phone": user.get("phone"), "city": user.get("city"),
                   ...
                   "vehicle_reg": user.get("current_vehicle_number") or user.get("vehicle_reg")},
    ...
    if rental:
        ...
        out["rental"] = {"id": str(rental["_id"]), "package_id": rental.get("package_id"),
                         ...
                         "start_date": rental["start_date"], "vehicle_reg": rental.get("vehicle_reg")}
        out["account"] = acct
    return out
...
async def driver_login(body: DriverLogin):
    phone = re.sub(r"\s+", "", body.phone or "")
    await _check_lockout("driver:" + phone)
    ...
    token = authlib.create_access_token(str(u["_id"]), u.get("phone", ""))
...
async def create_rental_driver(body: NewRentalDriver, request: Request):
    ...
    await db.security_deposits.insert_one({"organization_id": org, "driver_id": did, "amount": DEFAULT_DEPOSIT,
...
async def delete_driver(did: str, request: Request):
    user = await get_user(request)
    require_role(user, ["city_manager", "staff"])
    res = await db.drivers.delete_one(org_filter(user, {"_id": oid(did)}))
